                ...
                using (var sqlCommand = new SqlCommand("Enqueue", sqlConnection))
                {
                    ...
                    using (var reader = sqlCommand.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            enqueueResult.Id = (long)reader[colDeviceId];
                            enqueueResult.DequeueIndex = (int)reader[colDequeueIndex];
                            enqueueResult.EnqueueIndex = (int)reader[colEnqueueIndex];
                            enqueueResult.Peek = (bool)reader[colPeek];
                            enqueueResult.Version = (int)reader[colVersion];
                            enqueueResult.MessageId = (int)reader[colMessageId];
                ...
                using (var sqlCommand = new SqlCommand("Commit", sqlConnection))
                {
                    ...
                    using (var reader = sqlCommand.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            deviceEntry.Id = (long)reader[colDeviceId];
                            deviceEntry.DequeueIndex = (int)reader[colDequeueIndex];
                            deviceEntry.EnqueueIndex = (int)reader[colEnqueueIndex];
                            deviceEntry.Peek = (bool)reader[colPeek];
                            deviceEntry.Version = (int)reader[colVersion];
                    ...
                    using (var reader = sqlCommand.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            if ((int) reader[colIsMessage] == 1)
                            {
                                ...
                                dequeueResult.Id = (long) reader[colDeviceId];
                                dequeueResult.DequeueIndex = (int) reader[colDequeueIndex];
                                dequeueResult.EnqueueIndex = (int) reader[colEnqueueIndex];
                                dequeueResult.Peek = (bool) reader[colPeek];
                                dequeueResult.Version = (int) reader[colVersion];
                                dequeueResult.MessageId = (int) reader[colMessageId];
